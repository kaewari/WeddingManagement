...
public class ApiHallController {
    ...
    public ResponseEntity<Hall> updateExistingHall(@PathVariable("hallId") int id,
        ...
        Hall existing = hallService.findById(id);
        if (!hall.getName().isEmpty()) existing.setName(hall.getName());
        if (!hall.getDescription().isEmpty()) existing.setDescription(hall.getDescription());
        if (!hall.getGuestUpTo().isEmpty()) existing.setGuestUpTo(hall.getGuestUpTo());
        if (!hall.getTableCount().isEmpty()) existing.setTableCount(hall.getTableCount());
        if (hall.getIsActive() != null) existing.setIsActive(hall.getIsActive());
    ...
    public ResponseEntity<HallPrice> updateExistingHallPrice(@PathVariable("hallPriceId") int id,
        ...
        if (!hallPrice.getPeriod().isEmpty()) existing.setPeriod(hallPrice.getPeriod());
