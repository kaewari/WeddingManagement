...
public class ApiEmployeeController {
    ...
    public ResponseEntity<Object> addEmployee(@RequestBody Map<String, String> e, @RequestPart(value = "file", required = false) MultipartFile file) {
        try {
            ...
            if (isEmployee != null) {
                UserGroup ug = this.userGroupService.findByName("Waiters");
...
//                return new ResponseEntity<>(message.toString(), HttpStatus.CONFLICT);
//            }
            Employee em = this.employeeService.getEmployeeByIdentityNumber(e.get("identityNumber"));
            if (em != null) {
                if (Objects.equals(em.getId(), employee.getId())) {
                    message.put("Msg", "This identity number existed");
                    return new ResponseEntity<>(message.toString(), HttpStatus.CONFLICT);
                }
            }
    ...
    public ResponseEntity<Object> deleteEmployee(@PathVariable(value = "id") int id) {
        try {
            if (this.employeeService.deleteEmployee(id)) {
                message.put("Msg", "Delete employee successfully");
                return new ResponseEntity<>(message.toString(), HttpStatus.OK);
            } else {
                message.put("Msg", "Delete employee failure");
                return new ResponseEntity<>(message.toString(), HttpStatus.CONFLICT);
            }
