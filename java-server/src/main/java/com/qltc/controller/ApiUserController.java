...
public class ApiUserController {
    ...
    @CrossOrigin
    public ResponseEntity<Object> addUser(@RequestBody User u, @RequestPart(value = "file", required = false) MultipartFile file) {
        try {
...
//            }
            ...
            if (isUser != null) {
                UserGroup ug = this.userGroupService.findByName("Users");
                if (this.userGroupService.addUserToGroup(this.userService.getUserById(isUser.getId()), ug)) {
                    ...
                    userGroupPermissions.forEach(ugp -> {
                        ...
                        userPermission.setAllows(Boolean.TRUE);
        ...
        try {
            ...
            if (user != null) {
                ...
                if (u.getPassword() != null
                        && u.getNewPassword() != null
                        && u.getConfirmPassword() != null) {
                    if (u.getPassword().isBlank()) {
                        return new ResponseEntity<>("Please fill out your current password", HttpStatus.NOT_FOUND);
                    }
                    if (u.getNewPassword().isBlank()) {
                        return new ResponseEntity<>("Please fill out your new password", HttpStatus.NOT_FOUND);
                    }
                    if (u.getConfirmPassword().isBlank()) {
                        return new ResponseEntity<>("Please fill out your confirm password", HttpStatus.NOT_FOUND);
                    }
                    if (!this.passEncoder.matches(u.getPassword(), user.getPassword())) {
                        return new ResponseEntity<>("Your current password does not match", HttpStatus.NOT_FOUND);
                    } else {
                        if (!u.getConfirmPassword().equals(u.getNewPassword())) {
                            return new ResponseEntity<>("New password and confirm password do not match", HttpStatus.NOT_FOUND);
                        }
                    }
                    user.setPassword(this.passEncoder.encode(u.getNewPassword()));
                }
