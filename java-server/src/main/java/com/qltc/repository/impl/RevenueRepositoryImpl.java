...
public class RevenueRepositoryImpl implements RevenueRepository {
    ...
    @Override
    public List<Object[]> getStatsRevenueByYear(Map<String, String> params) {
        Session session = this.factory.getObject().getCurrentSession();
        String queryString = "select quarter(o.createdDate) as 'quarter', sum(odd.quantity*odd.price) + sum(ohd.price) + sum(osd.quantity*osd.price) as 'total'\n"
                + "                from orders o, order_dish_details odd, order_halls_details ohd, order_services_details osd\n"
                + "                where o.id = odd.orderId and o.id = ohd.orderId and o.id=osd.orderId and year(o.createdDate) = ?1\n"
                + "                group by quarter(o.createdDate)\n"
                + "                order by 1 asc;";
        String year = null;
        if (params != null) {
            try {
    ...
    @Override
    public List<Object[]> getStatsRevenueByDateRange(Map<String, String> params) {
        Session session = this.factory.getObject().getCurrentSession();
        String queryString = "select o.id, sum(odd.quantity*odd.price) + sum(ohd.price) + sum(osd.quantity*osd.price) as 'total'\n"
                + "                from orders o, order_dish_details odd, order_halls_details ohd, order_services_details osd\n"
                + "                where o.id = odd.orderId and o.id = ohd.orderId and o.id=osd.orderId and \n"
                + "                (o.createdDate between ?1 and ?2)\n"
                + "                group by o.id\n"
                + "                order by 1 desc;";
